...
def plot_metrics(metrics, output_path):
    ...
    epochs = range(1, metrics['Epoch'] + 1)
...
# 计算并打印模型设定参数和性能参数
def get_model_parameters_and_performance(metrics):
    model_params = {
...
# 将参数和图像写入Excel文件
def write_to_excel(metrics, output_path):
    ...
    # 写入参数
    model_params = get_model_parameters_and_performance(metrics)
    ...
    ws2.add_image(img1, 'E2')
    ws2.add_image(img2, 'E22')
    ws2.add_image(img3, 'E42')
...
write_to_excel(metrics, output_path)
